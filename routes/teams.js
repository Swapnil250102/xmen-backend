...
router.get('/', async (req, res) => {
  try {
    ...
    const teamData = await Promise.all(teams.rows.map(async (team) => {
      const members = await pool.query('SELECT id, name, color, mutant_class FROM mutants WHERE $1 = ANY(affiliations)', [team.id])
...
router.get('/:id', async (req, res) => {
  try {
    ...
    const members = await pool.query('SELECT id, name, color, mutant_class, power_types FROM mutants WHERE $1 = ANY(affiliations)', [req.params.id])
